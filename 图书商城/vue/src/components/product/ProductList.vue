...
<template>
  <div class="product-list">
      ...
      <div slot="header" class="clearfix">
        ...
        <el-form :model="ruleForm" class="el-form" ref="ruleForm" inline>
          <el-form-item label="图书名称：" prop="name">
            <el-input v-model="ruleForm.name" placeholder="图书名称"></el-input>
          </el-form-item>
          <el-form-item label="图书货号：" prop="goodsNo">
            <el-input v-model="ruleForm.goodsNo" placeholder="图书货号"></el-input>
          </el-form-item>
          <el-form-item label="图书分类：" prop="sort">
          ...
          <el-form-item label="出版社：" prop="brand">
          ...
          <el-form-item label="重磅状态：" prop="new_status">
    ...
    <el-card class="box-card product-list-card" shadow="never">
      ...
      <div>
        <el-table :data="tableData" height="500px" border slot="empty" stripe highlight-current-row style="width: 100%">
            ...
            <el-table-column label="图书图片" align="center" width="150">
            ...
            <el-table-column label="图书名称" align="center" width="300">
              <template slot-scope="scope" >
            ...
            <el-table-column label="分类/品牌" align="center" width="140">
              <template slot-scope="scope">
                ...
                <p>出版社：{{tableData[scope.$index].productBrand}}</p>
              </template>
            </el-table-column>
            ...
            <el-table-column label="标签" align="center" width="140">
              <template slot-scope="scope">
                ...
                <p>重磅：<el-switch @change="isNewState(scope.$index)" v-model="tableData[scope.$index].isNew"/></p>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" min-width="150">
              <template slot-scope="scope">
                <div>
                  <el-button size="mini" type="danger" slot="reference" @click="purchase(scope.$index)" style="margin-bottom: 20px!important;">图书采购</el-button>
                </div>
                <div>
    ...
    <el-dialog title="编辑图书" class="chang-product" :visible.sync="dialogFormVisible" center v-if="tableData[handleIndex]">
      <el-form class="el-form" inline :rules="alterRules" :model="alterForm" ref='changeFrom'>
        <el-form-item label="图书名称：" prop="productName">
          <el-input  v-model="alterForm.productName" placeholder="图书名称"></el-input>
        </el-form-item>
        <el-form-item label="图书分类：" prop="productType">
        ...
        <el-form-item label="图书进价：" prop="inPrice">
          <el-input type='number' min=0 v-model="alterForm.inPrice" placeholder="图书进价"></el-input>
        </el-form-item>
        <el-form-item label="出版社：" prop="productBrand">
        ...
        <el-form-item label="图书售价：" prop="outPrice">
          <el-input type='number' min=0 v-model="alterForm.outPrice" placeholder="图书售价"></el-input>
        </el-form-item>
        ...
        <el-form-item label="图书描述：" prop="productDescribe">
          <el-input type="textarea" autosize v-model="alterForm.productDescribe" placeholder="图书描述"></el-input>
        </el-form-item>
        <el-form-item label="图书图片：" prop="productUrl">
          <el-upload
    ...
    <el-dialog title="图书采购" class="chang-purchase" :close-on-click-modal="false" :visible.sync="purchaseFormVisible" width="500px" center>
      <el-form class="info-form" inline :model="purchaseForm" :rules="purchaseRules" ref='changeFrom'>
        <el-form-item label="图书编号：" prop="productNo">
          <el-input size="small" readonly v-model="purchaseForm.productNo" placeholder="图书编号"></el-input>
        </el-form-item>
        <el-form-item label="图书名称：" prop="productName">
          <el-input size="small" readonly v-model="purchaseForm.productName" placeholder="图书名称"></el-input>
        </el-form-item>
  ...
  export default {
    ...
    data(){
      return{
        ...
        purchaseFormVisible:false,  //图书采购界面
        handleIndex:0,
        ...
        alterRules: {
          productName: [{ required: true, message: '请输入图书名称', trigger: 'blur' },
            { min: 1, max: 30, message: '名称长度不允许超过 30 个字符', trigger: 'blur' }],
          outPrice:[{ required: true, message: '请输入图书售价', trigger: 'blur' }],
          inPrice:[{ required: true, message: '请输入图书进价', trigger: 'blur' }],
          productDescribe: [{ required: true, message: '请输入图书描述', trigger: 'blur' },
          ...
          if (valid) {
            ...
            this.$http.post('/product/update',this.$qs.stringify(this.alterForm)).then((rep)=>{
              ...
              if(rep.data.code===200){
                ...
                this.$notify({title: '成功', message: '图书信息更新成功', type: 'success'});
              }else{
                this.$msg.error(rep.data.message)
              }
        ...
        this.$http.post('/product/update?productId='+this.tableData[index].productId+'&isNew='+this.tableData[index].isNew).then((rep)=>{
          ...
          if(rep.data.code===200){
            if(this.tableData[index].isNew){
              this.reqInfo();
              this.$notify({title: '成功', message: this.tableData[index].productName+'\t设为重磅', type: 'success'});
            }else{
              this.$notify({title: '成功', message: this.tableData[index].productName+'\t设为重磅正常', type: 'success'});
            }
          }
        }).catch((err)=>{loading.close();this.$msg.error(err)})
      },
        ...
        if(queryData.name.length===0 &&
          ...
          for(let i=0;i<data.length;i++){
            ...
            if(queryData.new_status.length!==0){
              if(queryData.new_status==="重磅" && data[i].isNew){
                this.tableTemp.push(data[i]);
                count++;
        ...
        this.$confirm('此操作将永久删除该图书, 是否继续?', '提示', {
          ...
          this.$http.post('/product/delete?productId='+this.tableData[index].productId).then((rep)=>{
            if(rep.data.code === 200){
              ...
              this.$notify({title: '成功', message: name+'\t图书移除成功', type: 'success'});
            }
          }).catch((err)=>{
            this.$msg.error(err);
          })
